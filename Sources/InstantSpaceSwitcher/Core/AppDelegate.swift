...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func registerHotkey(for identifier: HotkeyIdentifier, combination: HotkeyCombination) {
        ...
        HotKeyManager.shared.register(identifier: identifier, combination: combination) {
            [weak self] in
    ...
    private func performSpaceSwitch(_ direction: ISSDirection) {
        performSpaceSwitch { iss_switch(direction) }
    }

    private func performSpaceSwitchToIndex(_ index: UInt32) {
        performSpaceSwitch { iss_switch_to_index(index) }
    }

    private func performSpaceSwitch(_ switchAction: @escaping () -> Bool) {
        SwitchOverlayController.shared.performTransition(switchAction) { [weak self] switched in
            self?.handleSpaceSwitchResult(switched)
        }
    }

    private func handleSpaceSwitchResult(_ switched: Bool) {
        guard switched else {
            NSSound.beep()
            return
        }
